...
do
    ...
    psql --host localhost --port 5432 --dbname postgres --user postgres --tuples-only --command "SELECT ${schema_name}.extract_table_as_xml('${table_name}', '${schema_name}');" \
    | xmllint --format - | sed 1d > "${output_directory%/}/${table_name}.xsd"
done
